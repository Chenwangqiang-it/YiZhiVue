<template>
    <div class="app-container" >
         <template>
            ...
            <el-table-column width="200" align="left" prop="mailName" show-overflow-tooltip="true" label="公司名称"></el-table-column>
            ...
            <el-table-column width="80" align="center"  show-overflow-tooltip="true" prop="express" label="快递公司"></el-table-column>
            <el-table-column width="170" align="center"  show-overflow-tooltip="true" prop="number" label="快递单号"></el-table-column>
            <el-table-column width="120" align="left" show-overflow-tooltip="true" prop="describes" label="详细"></el-table-column>
            <el-table-column width="80" align="center" label="状态">
                <tempate slot-scope="scope">
                    {{scope.row.state==0?"待修改":scope.row.state==1?"待邮寄":scope.row.state==2?"收件确认":scope.row.state==3?"邮寄完成":""}}
                </tempate>
            </el-table-column>
            <el-table-column align="center" v-if="historyMailQuery.state==2" show-overflow-tooltip="true" prop="gmtCreate" label="发起时间"></el-table-column>
            <el-table-column align="center" width="150"  show-overflow-tooltip="true" prop="mDate" label="邮寄时间"></el-table-column>
            <el-table-column width="80" align="center" label="操作">
                <template slot-scope="scope">
                    <el-button v-if="historyMailQuery.state==2&&(roles.jurisdiction==0||roles.jurisdiction==5||roles.jurisdiction==6)" @click="to(scope.row.mailId)" type="primary" size="mini">处理</el-button>
                    <el-button v-if="!historyMailQuery.state||(historyMailQuery.state==2&&(roles.jurisdiction!=0&&roles.jurisdiction!=5&&roles.jurisdiction!=6))" @click="to2(scope.row.mailId)" type="primary" size="mini">记录</el-button>
                </template>
            </el-table-column>
            </el-table>
            <el-pagination
...
export default {//定义变量和初始值
    ...
    methods:{
        ...
        getList(page =1){
            if(this.$route.path=="/mail/receipt"){
                 this.historyMailQuery.state=2
                this.style="width:98%"
            }else{
                this.historyMailQuery.state=undefined
                this.style="width:97%"
            }
        ...
        to2(id){
            let routeData = this.$router.resolve({
                path: '/history/mail/'+id
            })
            // this.$router.push({name:'/audit/update/',query: {id,cid,url,isaudit}})
            if(this.win4!=null){
                 this.win4.close()
            }
            this.win4=window.open(routeData.href,'win4','width=600px,height=800px,top=100px,left=700px,resizable=yes,scrollbars')
        },
